...
export async function fetchFieldSuggestions(fieldName: string, existingValue: string | undefined, format: 'paragraph' | 'bulleted' = 'paragraph', index?: number, context?: any) {
  const token = typeof window !== 'undefined' ? localStorage.getItem('token') : undefined
  const body: any = { fieldName, format }
