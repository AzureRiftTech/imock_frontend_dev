...
export default function InterviewResultByIdPage({ params }: { params: Promise<{ interview_id: string; result_id: string }> }) {
    const { interview_id: interviewId, result_id: resultId } = use(params)
    ...
    useEffect(() => {
        const fetchResultById = async () => {
            try {
                const res = await api.get(`/answer-analysis/result/${resultId}`)
                if (res.data.success) {
        ...
        return (
            <div className="flex-1 min-h-screen flex flex-col items-center justify-center bg-zinc-950 text-white gap-6">
                ...
                <div className="space-y-2 text-center">
                    <h2 className="text-2xl font-bold tracking-tight">Fetching Saved Result</h2>
                    <p className="text-zinc-500">Loading your saved performance report...</p>
                </div>
            </div>
        )
    }
                ...
                <div className="max-w-md w-full bg-zinc-900 border border-white/10 rounded-[2.5rem] p-12 text-center shadow-2xl space-y-8">
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed">
                            {error || "We couldn't find the requested result."}
                        </p>
                    </div>
                    <Button
        ...
        <div className="min-h-screen bg-zinc-950 text-white pb-24 selection:bg-brand-500/30">
            ...
            <main className="max-w-7xl mx-auto px-6 py-12 space-y-10">
                        ...
                        <div className="p-10 lg:p-14 flex flex-col lg:flex-row gap-12 items-center lg:items-center">
                            <div className="flex-1 space-y-8 w-full">
                                <div className="space-y-4">
                                    <h1 className="text-4xl lg:text-5xl font-bold tracking-tight">Saved Performance Report</h1>
                                    <div className="flex items-center gap-4">
                                        <div className="flex text-yellow-500">{[1,2,3,4,5].map(s => <Star key={s} className={`w-6 h-6 ${s <= (result.overall_score / 20) ? 'fill-current' : 'opacity-20'}`} />)}</div>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    <div className="px-5 py-2.5 rounded-full bg-zinc-800 border border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Mode: Saved Session</div>
                                </div>
                            </div>
                ...
                <div className="grid lg:grid-cols-2 gap-10">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[3rem] p-10 space-y-8">
                        ...
                        <div className="h-[400px] w-full mt-4 flex items-center justify-center">
                            <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                                    ...
                                    <Radar name="Skills" dataKey="A" stroke="#D97706" fill="#D97706" fillOpacity={0.5} />
                                </RadarChart>
                            </ResponsiveContainer>
                        </div>
                    </motion.div>
